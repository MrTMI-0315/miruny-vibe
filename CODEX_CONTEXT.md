...
- `npm run e2e` = Playwright 회귀 스모크(landing→prepare, run guard, run step advance, step3 CTA policy, done guard, done positive)
...
- E2E: `landing-to-prepare`, `run-guard-redirect`, `run-step-advance`, `step3-cta-policy`, `done-guard-redirect`, `done-positive` 스모크 추가
