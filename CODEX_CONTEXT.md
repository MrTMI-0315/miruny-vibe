...
- `npm run e2e` = Playwright 회귀 스모크(landing→prepare, run/prepare/done guard, run step advance, step3 CTA policy, done positive)
...
- E2E: `landing-to-prepare`, `run-guard-redirect`, `prepare-guard-redirect`, `run-step-advance`, `step3-cta-policy`, `done-guard-redirect`, `done-positive` 스모크 추가
